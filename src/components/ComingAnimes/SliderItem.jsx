...
export default function SliderItem({ data }) {
    return (<Flex position={'relative'} zIndex={'5'} justifyContent={'space-between'} borderRadius={'12px'} w={'100%'} overflow={'hidden'} height={'100%'} backgroundSize={'cover'} backgroundRepeat={'no-repeat'} p={'50px'} backgroundImage={`url(${data.image_banner})`} _before={{
            content: "''",
            display: "block",
            width: "100%",
            height: "100%",
            ...
            zIndex: "10"
                ...
                <Box zIndex={'11'} maxW={'50%'} position={'relative'}>
                    <Text display={'inline'} fontSize={'xl'} backgroundColor={'blue.600'} color={'textPrimary'} p={'12px  18px'} borderRadius={'10px'}>
                    ...
                    <Text size={'xl'} mt={'20px'} mb={'20px'} noOfLines={[1, 2, 3]}>{data.description}</Text>
                        <Button variant={'primary'} colorScheme={'blue'} as={NextLink} href={data.link} size={'lg'} maxW={'220px'} position={'absolute'} bottom={0}>
                            Дивитися
                        </Button>
