...
function useVolume({
  ...
  const [volume, setVolume] = (0, import_react5.useState)(initialVolume != null ? initialVolume : playback.volume);
  ...
  const updateCurrentVolume = (0, import_react5.useCallback)((vol) => {
...
function useTimeline({
  updateInterval = 250,
  isSlidingRef = import_react6.default.createRef()
} = {}) {
  const { playbackRef, playerRef } = useLimeplay();
  const playback = playbackRef.current;
  const player = playerRef.current;
  const currentTimerId = (0, import_react6.useRef)(-1);
  ...
  const [currentProgress, setCurrentProgress] = (0, import_react6.useState)(0);
  const [isLive, setIsLive, isLiveRef] = use_state_ref_default(false);
  const [liveLatency, setLiveLatency] = (0, import_react6.useState)(0);
  const [seekRange, setSeekRange, seekRangeRef] = use_state_ref_default({
    start: 0,
    end: 0
  });
  const updateCurrentTime = (0, import_react6.useCallback)((time) => {
...
var import_react7 = require("@use-gesture/react");
...
function useTimelineDrag({
  ...
  (0, import_react7.useDrag)(dragHandler, {
    target: ref,
    enabled: !disabled
  });
  return {
...
var import_react9 = require("@use-gesture/react");
...
function useTimelineHover({
  ...
  const [isMoving, setIsMoving] = (0, import_react10.useState)(false);
  const [value, setValue] = (0, import_react10.useState)(0);
  const { disabled } = sliderHandlerConfig;
  const dragHandler = ({
    type,
    active,
...
var import_react11 = require("@use-gesture/react");
...
function useSliderEvents({
  ...
  const [isInside, setIsInside] = (0, import_react12.useState)(false);
  const [isKeying, setIsKeying] = (0, import_react12.useState)(false);
  ...
  const hoverHandler = (0, import_react12.useCallback)(
    ({ hovering, type }) => {
...
function usePiP({
  ...
  const [isPiPSupported, setIsPiPSupported] = (0, import_react13.useState)(false);
  const [isPiPAllowed, setIsPiPAllowed] = (0, import_react13.useState)(false);
  const [pipWindow, setPipWindow] = (0, import_react13.useState)(
    null
  );
...
function useVideoTracks({
  ...
  const { playerRef } = useLimeplay();
  const player = playerRef.current;
  const loadMode = (0, import_react14.useRef)(player == null ? void 0 : player.getLoadMode());
...
function useAudioTracks() {
  const { playerRef } = useLimeplay();
  const player = playerRef.current;
  const [tracks, setTracks] = (0, import_react15.useState)([]);
  const [selectedTrack, setSelectedTrack, selectedTrackRef] = use_state_ref_default(null);
  const previousTrack = (0, import_react15.useRef)(null);
  const selectTrack = (0, import_react15.useCallback)(
  ...
  const updateQualityHandler = (0, import_react15.useCallback)(() => {
...
function useBufferInfo({ updateInterval = 1e3 } = {}) {
  const [bufferInfo, setBufferInfo] = (0, import_react16.useState)([]);
  const currentTimerId = (0, import_react16.useRef)(-1);
  const { playbackRef, playerRef } = useLimeplay();
  const player = playerRef.current;
  const playback = playbackRef.current;
...
function useFullScreen({
  ...
  const [isFullScreenSupported, setIsFullScreenSupported] = (0, import_react17.useState)(false);
  ...
  const fullscreenEventHandler = (0, import_react17.useCallback)(
    (_event) => {
      var _a;
      if (screenfull_default.isEnabled) {
...
function useOrientation({
  ...
  const [orientation2, setOrientation] = (0, import_react18.useState)(
...
function usePlayback() {
  ...
  const { playbackRef } = useLimeplay();
  const playback = playbackRef.current;
  const [isEnded, setIsEnded] = (0, import_react20.useState)(false);
  const [isRepeat, setIsRepeat] = (0, import_react20.useState)(false);
...
function useTextTracks() {
  ...
  const [selectedTrack, setSelectedTrack, selectedTrackRef] = use_state_ref_default(null);
  const previousTrack = (0, import_react21.useRef)(null);
  const [isVisible, setIsVisible] = (0, import_react21.useState)(false);
  const selectTrack = (0, import_react21.useCallback)((track) => {
